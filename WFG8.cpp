...
double WFG8::next_double( const double bound = 1.0 )
{
  assert( bound > 0.0 );
...
vector< double > WFG8::WFG_8_random_soln( const int k, const int l )
{
...
// Evaluacion de un individuo WFG8
void WFG8::evaluate (vector<double> &X, vector<double> &Obj) {
...
// Clonacion de un individuo WFG8
//Individual* WFG8::clone (void) const {
//  return new WFG8();
//}
...
double WFG8::correct_to_01 (const double &a, const double &epsilon) {
...
vector<double> WFG8::subvector (const vector<double> &v, const int head, const int tail) {
...
vector<double> WFG8::calculate_x (const vector<double> &t_p, const vector<short> &A) {
...
vector<double>  WFG8::calculate_f (const double &D, const vector<double> &x, const vector<double> &h,
...
double WFG8::b_param (const double &y, const double &u, const double &A, const double &B, const double &C) {
	const double v = A - (1.0 - 2.0 * u) * fabs(floor(0.5 - u) + A);
	return correct_to_01(pow(y, B + (C - B) * v));
}

double WFG8::s_linear (const double &y, const double &A) {
	return correct_to_01(fabs(y - A) / fabs(floor(A - y) + A));
}

double WFG8::r_sum (const vector<double> &y, const vector<double> &w) {
...
double WFG8::concave(const vector<double> &x, const int m) {
...
/******************************************************************************
 * Funciones propias del problema WFG8
 * ****************************************************************************/

// Primera transicion para WFG8
vector<double> WFG8::WFG8_t1 (const vector<double> &y, const int k) {
...
// Segunda transicion para WFG8
vector<double> WFG8::WFG8_t2 (const vector<double> &y, const int k) {
...
// Tercera transicion para WFG8
vector<double> WFG8::WFG8_t3 (const vector<double> &y, const int k, const int M) {
...
// Superficie para WFG8
vector<double> WFG8::WFG8_shape (const vector<double> &t_p) {
...
// "1,1,1,..."
vector<short> WFG8::WFG_create_A (const int M, const bool degenerate) {
...
vector<double> WFG8::WFG_calculate_f (const vector< double >& x, const vector< double >& h) {
...
vector< double > WFG8::WFG_normalise_z( const vector< double >& z )
{
	vector< double > result;
